...
def check_user_limit(author, minutes=1):
    user_ref = db.collection("users").document(author)
    ...
    if user_doc.exists:
        ...
        last_msg_time = user_data.get("lastMessageAt")
        if last_msg_time and isinstance(last_msg_time, datetime):
            if now - last_msg_time < cooldown:
                remaining = (cooldown - (now - last_msg_time)).seconds
                return False, remaining
    # обновляем время
    user_ref.set({"lastMessageAt": now}, merge=True)
    return True, 0
...
def add_thread():
    ...
    try:
        ...
        allowed, wait_seconds = check_user_limit(author, minutes=1)
        if not allowed:
            return jsonify({
                "error": f"Подождите ещё {wait_seconds//60} мин. {wait_seconds%60} сек. перед следующим сообщением"
            }), 429
...
def add_comment(thread_id):
    ...
    try:
        ...
        if not allowed:
            return jsonify({
                "error": f"Подождите ещё {wait_seconds//60} мин. {wait_seconds%60} сек. перед следующим комментарием"
            }), 429
